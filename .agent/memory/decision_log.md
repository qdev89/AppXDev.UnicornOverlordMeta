...
| 2026-08-17 | Authentic Wiki Data Engine Overhaul | Initial data files had placeholder skills, missing AP/PP costs, and unmapped equipment items. | Rebuilt `skills.ts`, `items.ts`, and `classes.ts` using authentic wiki data pipelines. |
| 2026-08-18 | Universal Hero Frame & Character Portrait Engine | Character avatars fell back to raw emojis when `image` fields were missing on classes or heroes. | Built `<HeroFrame />` component and `HERO_IMAGE_MAP` in `squadUtils.ts`, unifying hero portraits across all components. |
